...
def isUnfair(opt_Ks, x_train, y_train, _test, predicted_label, protected_indicies, protected_value_range, epsilon, max_flips):
    for k in opt_Ks:
        if prediction_change(k, x_train, y_train, x_test, predicted_label, protected_indicies, protected_value_range, epsilon, max_flips) == True:
            return True
    return False


def prediction_change(K, x_train, y_train, x_test, predicted_label, protected_indicies, protected_combinations, epsilon, max_flips):
...
def getParameters():
    if len(sys.argv) < 8:
        print("args = (filename, down_sampling_paras, cross_validation_fold, kset, protected_indicies, pertubation, pertubation_indicies, max_flip)")
        exit()
    filename = sys.argv[1]
    down_sampling_paras = ast.literal_eval(sys.argv[2])
    cross_validation_fold = int(sys.argv[3])
    kset = ast.literal_eval(sys.argv[4])
    protected_indicies = ast.literal_eval(sys.argv[5])
    pertubation = float(sys.argv[6])
    pertubation_indicies = ast.literal_eval(sys.argv[7])
    max_flips = int(sys.argv[8])
    ...
    return XTrain, yTrain, X_test, y_test, down_sampling_paras, cross_validation_fold, kset, protected_indicies, pertubation, pertubation_indicies, max_flips


if __name__ == "__main__":
    XTrain, yTrain, X_test, y_test, down_sampling_paras, cross_validation_fold, kset, protected_indicies, pertubation, pertubation_indicies, max_flips = getParameters()
    ...
    epsilon = pertubation * (np.max(down_x_train, axis = 0) - np.min(down_x_train, axis = 0))
    ...
    for i, x_test in enumerate(X_test):
        if isUnfair(opt_Ks, down_x_train, down_y_train, x_test, predictions[i], protected_indicies, protected_combinations, epsilon, max_flips) == True:
    ...
    print("#test data = ", test_cnt, "total time =", total_time_seconds, "s, avg time = ", total_time_seconds / test_cnt, "s")
